...
import {
  vNetVisaku, styles, netInbox, timer, types, ModalPopUp, MaterialIndicator,
...
import VisakuHeader from './PropsHeaderComponent/VisakuHeader'


export default class VisakuVirtualScreen extends Component {
  _isMounted = false;
  state= {
    ArrCode: [],
    input: '',
  ...
  _onRetreiveValueDataCodeVoucher = async () => {
    try {
      ...
      let results = await axios.get(vNetVisaku())
  ...
  render() {
    ...
    return (
      <Container>
      <VisakuHeader {...this.props} onPress={this._onReloadScreenAndData}/>
      <ReloadScreen
